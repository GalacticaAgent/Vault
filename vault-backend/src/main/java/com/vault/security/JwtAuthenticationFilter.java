 ...
 * 1. 从请求头中提取 JWT Token
 * 2. 验证 Token 的有效性
 * 3. 解析 Token 获取用户信息（userId、username、role）
 * 4. 将用户信息存入 Spring Security 上下文
 * 5. 后续 Controller 可通过 SecurityUtil 获取当前登录用户信息
 * <p>
 * 使用示例：
 * 前端请求头: Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
 * Controller中获取: Long userId = SecurityUtil.getCurrentUserId();
 */
@Slf4j
@Component
